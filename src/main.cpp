...
int main(int argc, char *argv[])
{
            ...
            } else
            {
                ...
                if (X11Extras::getInstance()->display() == NULL)
                {
                    appLogger.LogError(QObject::tr("Display string \"%1\" is not valid.").arg(cmdutility.getDisplayString()),
                                       true, true);
                    // errorstream << QObject::tr("Display string \"%1\" is not valid.").arg(cmdutility.getDisplayString())
                    // << endl;
...
#if defined(Q_OS_UNIX)
    myappTranslator.load(QString("antimicro_").append(targetLang),
                         QApplication::applicationDirPath().append("/../share/antimicro/translations"));
#elif defined(Q_OS_WIN)
    myappTranslator.load(QString("antimicro_").append(targetLang),
                         QApplication::applicationDirPath().append("\\share\\antimicro\\translations"));
#endif
...
#if (defined(Q_OS_UNIX) && defined(WITH_UINPUT) && defined(WITH_XTEST)) || defined(Q_OS_WIN)
    ...
    QObject::connect(joypad_worker, SIGNAL(joysticksRefreshed(QMap<SDL_JoystickID, InputDevice *> *)), w,
                     SLOT(fillButtons(QMap<SDL_JoystickID, InputDevice *> *)));
    ...
    QObject::connect(&a, SIGNAL(aboutToQuit()), &PadderCommon::mouseHelperObj, SLOT(deleteDeskWid()), Qt::DirectConnection);
...
#ifdef USE_SDL_2
    QObject::connect(w, SIGNAL(mappingUpdated(QString, InputDevice *)), joypad_worker,
                     SLOT(refreshMapping(QString, InputDevice *)));
    QObject::connect(joypad_worker, SIGNAL(deviceUpdated(int, InputDevice *)), w,
                     SLOT(testMappingUpdateNow(int, InputDevice *)));
