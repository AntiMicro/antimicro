    ...
    , helper(button)
{
    ...
    connect(this, SIGNAL(advancedDialogOpened()), ui->virtualKeyMouseTabWidget,
            SLOT(establishVirtualKeyboardAdvancedSignalConnections()));
    connect(this, SIGNAL(advancedDialogOpened()), ui->virtualKeyMouseTabWidget,
...
ButtonEditDialog::~ButtonEditDialog() { delete ui; }
...
void ButtonEditDialog::changeToggleSetting() { button->setToggle(ui->toggleCheckBox->isChecked()); }

void ButtonEditDialog::changeTurboSetting() { button->setUseTurbo(ui->turboCheckBox->isChecked()); }
...
void ButtonEditDialog::openAdvancedDialog()
{
    ...
    connect(dialog, SIGNAL(finished(int)), ui->virtualKeyMouseTabWidget,
            SLOT(establishVirtualKeyboardSingleSignalConnections()));
    ...
    connect(ui->virtualKeyMouseTabWidget, SIGNAL(selectionMade(JoyButtonSlot *)), dialog,
            SLOT(placeNewSlot(JoyButtonSlot *)));
    connect(ui->virtualKeyMouseTabWidget, SIGNAL(selectionMade(int, unsigned int)), this,
            SLOT(createTempSlot(int, unsigned int)));
...
{
    QMetaObject::invokeMethod(&helper, "setAssignedSlot", Qt::BlockingQueuedConnection, Q_ARG(int, tempslot->getSlotCode()),
...
void ButtonEditDialog::clearButtonSlots() { QMetaObject::invokeMethod(button, "clearSlotsEventReset", Q_ARG(bool, false)); }
